defmodule Insightdb.CommandTest do
  ...
  test "normal http_command flow" do
    ...
    with_mocks([MongoMocks.gen(mock_db_key), HttpCommandMocks.gen()]) do
      ...
      %{Constant.field_status => "done"} = Command.find_cmd(cmd_id)
      assert %{"cmd_schedule_error" => [], "cmd_schedule_result" => result} = MongoMocks.get_db(mock_db_key)
  ...
  test "mongo exceptions" do
    ...
    mock_db_key = MongoMocks.setup_mock_db(123, :http_command, Keyword.put(@cmd_config_1, :verb, :get1))
    with_mocks([gen_mongo_mocks_and_replace.(mock_db_key), HttpCommandMocks.gen()]) do
      assert_raise RuntimeError, ~r/^can not find cmd with id .+$/, fn ->
        Command.find_cmd(123)
      end
      assert_raise RuntimeError, "oops!", fn ->
        Command.save_cmd_result(123, %{})
      end
      fun = fn ->
        Command.update_cmd_status_and_save_error(123, "errormsg")
      end
